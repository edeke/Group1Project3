...
Shader "Shader Forge/Examples/Painted_Environment_Toon_Shader" {
    Properties {
        ...
        _WearExponent ("Wear Exponent", Float ) = 1
    }
    SubShader {
        Tags {
            "RenderType"="Opaque"
...
////// Emissive:
                ...
                float node_3943 = saturate((node_1468.r+node_1468.g+node_1468.b));
                float node_315 = saturate((pow((node_3943+_WearThreshold),_WearExponent)-_WearThreshold));
                float3 node_277 = lerp(_Diffuse_var.rgb,node_2854,node_315);
                float3 emissive = (node_277*UNITY_LIGHTMODEL_AMBIENT.rgb);
                ...
                float3 finalColor = emissive + (_LightColor0.rgb*(node_277*node_2105));
...
////// Lighting:
                ...
                float node_5169 = 0.0;
                float node_2105 = (lerp(_MinLight,_MaxLight,saturate((node_5169 + ( (0.5*dot(lightDirection,normalDirection)+0.5 - node_2384) * (1.0 - node_5169) ) / ((_Reach+_Smoothness) - node_2384))))*attenuation);
                float3 finalColor = (_LightColor0.rgb*(node_277*node_2105));
...
/////// Vectors:
                ...
                float node_315 = saturate((pow((node_3943+_WearThreshold),_WearExponent)-_WearThreshold));
                float3 node_277 = lerp(_Diffuse_var.rgb,node_2854,node_315);
                o.Emission = (node_277*UNITY_LIGHTMODEL_AMBIENT.rgb);
