...
Shader "Shader Forge/Examples/Beam_water" {
    Properties {
        ...
        _WearThreshold ("Wear Threshold", Range(-2, 2)) = 0.1903217
...
////// Emissive:
                ...
                float node_6174 = saturate((_Normals_var.r+0.5));
                float3 node_9324 = lerp(_Diffuse_var.rgb,node_5122,saturate((pow((_WearThreshold+node_6174),_WearPower)-_WearThreshold)));
                float3 node_7231 = lerp((node_9324*_WetColor.rgb),node_9324,saturate((saturate((i.posWorld.g-_WaterHeight))*_Wetlinesharpness)));
                float3 emissive = ((node_7231*UNITY_LIGHTMODEL_AMBIENT.rgb)+(_Highlight.rgb*(1.0-max(0,dot(normalDirection, viewDirection)))));
...
/////// Vectors:
                ...
                float3 node_9324 = lerp(_Diffuse_var.rgb,node_5122,saturate((pow((_WearThreshold+node_6174),_WearPower)-_WearThreshold)));
                float3 node_7231 = lerp((node_9324*_WetColor.rgb),node_9324,saturate((saturate((i.posWorld.g-_WaterHeight))*_Wetlinesharpness)));
                o.Emission = ((node_7231*UNITY_LIGHTMODEL_AMBIENT.rgb)+(_Highlight.rgb*(1.0-max(0,dot(normalDirection, viewDirection)))));
