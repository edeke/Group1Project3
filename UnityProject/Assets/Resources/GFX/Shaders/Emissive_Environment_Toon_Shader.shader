...
Shader "Shader Forge/Examples/Emissive_Environment_Toon_Shader" {
    Properties {
        ...
        [MaterialToggle] _DistanceFade ("DistanceFade", Float ) = 0.3529412
        ...
        _EmissiveColor ("Emissive Color", Color) = (0.9219068,1,0.1911765,1)
...
////// Emissive:
                ...
                float3 emissive = (((_Diffuse_var.rgb*UNITY_LIGHTMODEL_AMBIENT.rgb)+(_EmissiveColor.rgb*_EmissiveIntensity*_EmissiveMap_var.r))+(_Highlight.rgb*(1.0-max(0,dot(normalDirection, viewDirection)))));
...
/////// Vectors:
                ...
                float4 _Diffuse_var = tex2D(_Diffuse,TRANSFORM_TEX(_Autotiletoscale_var, _Diffuse));
                float4 _EmissiveMap_var = tex2D(_EmissiveMap,TRANSFORM_TEX(i.uv0, _EmissiveMap));
                o.Emission = (((_Diffuse_var.rgb*UNITY_LIGHTMODEL_AMBIENT.rgb)+(_EmissiveColor.rgb*_EmissiveIntensity*_EmissiveMap_var.r))+(_Highlight.rgb*(1.0-max(0,dot(normalDirection, viewDirection)))));
