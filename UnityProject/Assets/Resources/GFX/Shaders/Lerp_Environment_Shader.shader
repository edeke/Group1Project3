...
Shader "Shader Forge/Examples/Lerp_Environment_Toon_Shader" {
    Properties {
        ...
        [MaterialToggle] _DistanceFade ("DistanceFade", Float ) = 0.5237889
        ...
        _LowColor ("Low Color", Color) = (0.5367647,0.4109027,0,1)
        _HighColor ("High Color", Color) = (0.5,0.5,0.5,1)
...
////// Emissive:
                ...
                float3 emissive = ((node_3954*UNITY_LIGHTMODEL_AMBIENT.rgb)+(_Highlight.rgb*(1.0-max(0,dot(normalDirection, viewDirection)))));
...
/////// Vectors:
                ...
                float4 _LerpMask_var = tex2D(_LerpMask,TRANSFORM_TEX(i.uv0, _LerpMask));
                float3 node_3954 = lerp(_Diffuse_var.rgb,lerp(_LowColor.rgb,_HighColor.rgb,_Diffuse_var.r),_LerpMask_var.r);
                o.Emission = ((node_3954*UNITY_LIGHTMODEL_AMBIENT.rgb)+(_Highlight.rgb*(1.0-max(0,dot(normalDirection, viewDirection)))));
