...
////// Lighting:
////// Emissive:
                float4 node_4799 = _Time + _TimeEditor;
                float2 node_2979 = ((i.uv0*(float2(0,1)+(float2(1,0)*_Tiling1)))+node_4799.g*float2(-0.006,0));
                float4 node_8028 = tex2D(_Texture,TRANSFORM_TEX(node_2979, _Texture));
                float2 node_1618 = ((i.uv0*(float2(0,1)+(float2(1,0)*_Tiling2)))+node_4799.g*float2(-0.0034,0));
                ...
                fixed4 finalRGBA = fixed4(finalColor,((((node_5679*_FresnelPostMult)*(saturate((node_2162+(saturate(((node_6706.b+_DustShift)*(node_7046.b+_DustShift)))*_Dustintensity)))*node_7511.g))*saturate((sceneZ-partZ)/_DepthFade))*_MasterOpacity));
