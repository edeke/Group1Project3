...
Shader "Shader Forge/Examples/Environment_Toon_Shader" {
    Properties {
        ...
        [MaterialToggle] _DistanceFade ("DistanceFade", Float ) = 1
...
////// Emissive:
                ...
                float node_2091 = (distance(_WorldSpaceCameraPos,objPos.rgb)/((((objScale.r+objScale.g)+objScale.b)/3.0)/_BaseScale));
                float node_6664 = saturate((_FadeDistance/node_2091));
                float3 emissive = (lerp( _Diffuse_var.rgb, lerp(_DistanceColor.rgb,_Diffuse_var.rgb,node_6664), _DistanceFade )*UNITY_LIGHTMODEL_AMBIENT.rgb);
...
/////// Vectors:
                ...
                float node_2091 = (distance(_WorldSpaceCameraPos,objPos.rgb)/((((objScale.r+objScale.g)+objScale.b)/3.0)/_BaseScale));
                float node_6664 = saturate((_FadeDistance/node_2091));
                o.Emission = (lerp( _Diffuse_var.rgb, lerp(_DistanceColor.rgb,_Diffuse_var.rgb,node_6664), _DistanceFade )*UNITY_LIGHTMODEL_AMBIENT.rgb);
