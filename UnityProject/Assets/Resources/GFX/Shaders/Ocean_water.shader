...
Shader "Shader Forge/Ocean_water" {
    Properties {
        ...
        _BeachDepth ("Beach Depth", Float ) = 2
        _OpacityFade ("Opacity Fade", Float ) = 0
        _Contrast ("Contrast", Float ) = 1
        _Contrastshift ("Contrast shift", Float ) = 0
    ...
    SubShader {
        ...
        Pass {
            ...
            uniform float _BeachDepth;
            uniform float _OpacityFade;
            uniform float _Contrast;
            uniform float _Contrastshift;
            struct VertexInput {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                float2 node_5692 = (o.uv0+node_1955.g*float2(0.02,0.03));
                float4 _height_var = tex2Dlod(_height,float4(TRANSFORM_TEX(node_5692, _height),0.0,0));
                float2 node_1560 = ((o.uv0*1.3)+node_1955.g*float2(0.03,0.04));
...
/////// Vectors:
                ...
                float2 node_3317 = (node_5627+node_1955.g*float2(0.002,0.003));
                float3 _NormalMap_copy_var = UnpackNormal(tex2D(_NormalMap_copy,TRANSFORM_TEX(node_3317, _NormalMap_copy)));
                float2 node_4995 = ((node_5627*_Tiling3)+node_1955.g*float2(0.002,0.012));
...
/////// Diffuse:
                ...
                float2 node_8160 = (node_5627+node_1955.g*float2(0.005,0.01));
                ...
                float3 node_7573 = lerp(_BeachTexture_var.rgb,lerp((_FadeColor.rgb*node_4826),node_4826,pow(saturate((sceneZ-partZ)/_Fadedepth),0.2)),saturate((sceneZ-partZ)/_BeachDepth));
                float3 diffuseColor = (pow((node_7573+_Contrastshift),_Contrast)-_Contrastshift);
                float3 diffuse = (directDiffuse + indirectDiffuse) * diffuseColor;
/// Final Color:
                float3 finalColor = diffuse;
                fixed4 finalRGBA = fixed4(finalColor,pow(saturate((sceneZ-partZ)/_OpacityFade),0.5));
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                o.tangentDir = normalize( mul( _Object2World, float4( v.tangent.xyz, 0.0 ) ).xyz );
                o.bitangentDir = normalize(cross(o.normalDir, o.tangentDir) * v.tangent.w);
                float4 node_7861 = _Time + _TimeEditor;
                float2 node_5692 = (o.uv0+node_7861.g*float2(0.02,0.03));
                float4 _height_var = tex2Dlod(_height,float4(TRANSFORM_TEX(node_5692, _height),0.0,0));
                float2 node_1560 = ((o.uv0*1.3)+node_7861.g*float2(0.03,0.04));
                float4 _height_copy_var = tex2Dlod(_height_copy,float4(TRANSFORM_TEX(node_1560, _height_copy),0.0,0));
                v.vertex.xyz += ((float3(0,1,0)*(_height_var.r*_height_copy_var.r))*(_VertexOffset/100.0));
                o.posWorld = mul(_Object2World, v.vertex);
                float3 lightColor = _LightColor0.rgb;
...
/////// Vectors:
                ...
                float4 node_7861 = _Time + _TimeEditor;
                float2 node_5627 = (i.uv0*_Tiling2);
                float2 node_3317 = (node_5627+node_7861.g*float2(0.002,0.003));
                float3 _NormalMap_copy_var = UnpackNormal(tex2D(_NormalMap_copy,TRANSFORM_TEX(node_3317, _NormalMap_copy)));
                float2 node_4995 = ((node_5627*_Tiling3)+node_7861.g*float2(0.002,0.012));
                float3 _NormalMap_var = UnpackNormal(tex2D(_NormalMap,TRANSFORM_TEX(node_4995, _NormalMap)));
                float3 normalLocal = (_NormalMap_copy_var.rgb+_NormalMap_var.rgb);
                float3 normalDirection = normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
...
/////// Diffuse:
                ...
                float3 directDiffuse = max( 0.0, NdotL) * attenColor;
                float4 _BeachTexture_var = tex2D(_BeachTexture,TRANSFORM_TEX(node_4995, _BeachTexture));
                float2 node_8160 = (node_5627+node_7861.g*float2(0.005,0.01));
                float4 _OceanTexture_copy_var = tex2D(_OceanTexture_copy,TRANSFORM_TEX(node_8160, _OceanTexture_copy));
                float4 _OceanTexture_var = tex2D(_OceanTexture,TRANSFORM_TEX(node_4995, _OceanTexture));
                float3 node_4826 = saturate(max(_OceanTexture_copy_var.rgb,_OceanTexture_var.rgb));
                float3 node_7573 = lerp(_BeachTexture_var.rgb,lerp((_FadeColor.rgb*node_4826),node_4826,pow(saturate((sceneZ-partZ)/_Fadedepth),0.2)),saturate((sceneZ-partZ)/_BeachDepth));
...
/// Final Color:
                float3 finalColor = diffuse;
                return fixed4(finalColor * pow(saturate((sceneZ-partZ)/_OpacityFade),0.5),0);
            }
            ENDCG
        }
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                VertexOutput o = (VertexOutput)0;
                o.uv0 = v.texcoord0;
                float4 node_6749 = _Time + _TimeEditor;
                float2 node_5692 = (o.uv0+node_6749.g*float2(0.02,0.03));
                float4 _height_var = tex2Dlod(_height,float4(TRANSFORM_TEX(node_5692, _height),0.0,0));
                float2 node_1560 = ((o.uv0*1.3)+node_6749.g*float2(0.03,0.04));
                float4 _height_copy_var = tex2Dlod(_height_copy,float4(TRANSFORM_TEX(node_1560, _height_copy),0.0,0));
                v.vertex.xyz += ((float3(0,1,0)*(_height_var.r*_height_copy_var.r))*(_VertexOffset/100.0));
                o.pos = mul(UNITY_MATRIX_MVP, v.vertex);
